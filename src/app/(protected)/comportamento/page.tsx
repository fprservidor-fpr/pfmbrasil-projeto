...
export default function ComportamentoPage() {
  ...
  const calculateMonthlyScore = (alunoId: string) => {
    const now = new Date();
  ...
  const getAlunoHistory = (alunoId: string) => {
    const now = new Date();
    const start = startOfMonth(now);
    const end = endOfMonth(now);

    const registros = comportamentos
      .filter(c => 
        c.aluno_id === alunoId && 
        new Date(c.created_at) >= start && 
        new Date(c.created_at) <= end
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <div className="flex flex-col items-end gap-1">
                ...
                <Badge variant="outline" className="border-zinc-800 text-zinc-400/50 px-4 py-1 text-[10px]">
                  ID SISTEMA: {selectedAluno.numero_ordem ? `${String(selectedAluno.numero_ordem).padStart(2, '0')}/${String(selectedAluno.ano_ingresso || 26).slice(-2)}` : selectedAluno.pre_matricula_id?.slice(0, 8).toUpperCase()}
                </Badge>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                <Card className="lg:col-span-4 bg-zinc-900/40 border-zinc-800 backdrop-blur-xl overflow-hidden rounded-3xl">
                  <div className={cn("h-2 w-full", (BEHAVIOR_LEVELS[selectedAluno.comportamento_atual || "EXCEPCIONAL"] || BEHAVIOR_LEVELS["EXCEPCIONAL"]).accent)} />
                  <CardContent className="p-8">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-zinc-950/50 border border-zinc-800 p-4 rounded-2xl text-center">
                        ...
                        <p className={cn("text-lg font-black", (BEHAVIOR_LEVELS[selectedAluno.comportamento_atual || "EXCEPCIONAL"] || BEHAVIOR_LEVELS["EXCEPCIONAL"]).textColor)}>
                          {selectedAluno.comportamento_atual || "EXCEPCIONAL"}
                        </p>
                      </div>
                      <div className="bg-zinc-950/50 border border-zinc-800 p-4 rounded-2xl text-center">
                        <p className="text-[10px] text-zinc-600 font-bold uppercase mb-1">Pontuação</p>
                        <p className="text-2xl font-black text-white">
                          {calculateMonthlyScore(selectedAluno.id)}
                        </p>
                      </div>
                    </div>
                  </CardContent>
                    ...
                    <CardContent>
                      <div className="h-64 mt-4">
                        <ResponsiveContainer width="100%" height="100%">
                          <AreaChart data={getAlunoHistory(selectedAluno.id)}>
                            <defs>
                  ...
                  <Card className="bg-zinc-900/40 border-zinc-800 backdrop-blur-xl rounded-3xl">
                    ...
                    <CardContent>
                      <div className="space-y-4 max-h-[400px] overflow-y-auto pr-4 custom-scrollbar">
                        {comportamentos.filter(c => c.aluno_id === selectedAluno.id).map((c, i) => (
                          <div 
                            key={c.id}
