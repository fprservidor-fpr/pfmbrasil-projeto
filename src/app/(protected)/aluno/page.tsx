  ...
  async function fetchData() {
    try {
      ...
      const [studentRes, eventsRes, behaviorRes, missoesRes, memberRes] = await Promise.all([
        ...
        supabase.from("cepfm_membros").select("*, cepfm_patrulhas(*)").eq("aluno_id", studentId).maybeSingle()
      ]);
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center gap-10">
          ...
          <div className="text-center md:text-left flex-1">
            ...
            <h1 className="text-4xl md:text-6xl font-black text-white tracking-tighter mb-2">
              Olá, <span className="text-transparent bg-clip-text bg-gradient-to-r from-violet-400 to-indigo-400">{student?.nome_guerra || student?.nome_completo?.split(" ")[0]}!</span>
            </h1>
            <p className="text-slate-400 text-lg font-medium max-w-xl">
              Bem-vindo ao seu portal oficial. Aqui você acompanha seu desempenho, materiais e eventos da Força Mirim.
            </p>
          </div>
        </div>
      </motion.div>
        ...
        >
          <div className="bg-gradient-to-br from-violet-600 to-indigo-700 rounded-[40px] p-8 text-white relative overflow-hidden shadow-2xl shadow-violet-900/20 group">
            ...
            <div className="grid grid-cols-1 gap-3 mt-8 relative z-10">
              <Link href="/aluno/cepfm" className="flex items-center gap-4 p-4 rounded-2xl bg-yellow-500/10 hover:bg-yellow-500/20 transition-all group/link border border-yellow-500/20 text-yellow-500">
              ...
              <Link href="/aluno/dossie" className="flex items-center gap-4 p-4 rounded-2xl bg-white/10 hover:bg-white/20 transition-all group/link border border-white/10">
          ...
          <div className="bg-slate-900/40 backdrop-blur-xl border border-white/5 rounded-[40px] p-8">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="text-sm font-black text-white uppercase tracking-widest">Dica do Dia</h3>
            </div>
            <p className="text-slate-400 text-sm italic leading-relaxed">
              "A disciplina é a ponte entre metas e realizações. Mantenha seu foco nos estudos e suas notas sempre em dia!"
            </p>
          </div>
        </motion.div>
      </div>
